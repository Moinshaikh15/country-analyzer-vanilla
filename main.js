...
function optInForm() {
  ...
  for (let i = 0; i < fields.length; i++) {
    let cInputDiv = document.createElement("div");
    let cLabel = document.createElement("label");
    let cInput = document.createElement("input");

    cLabel.htmlFor = fields[i];
    cLabel.innerHTML = fields[i];

    cInput.type = fields[i] === "Email" ? "email" : "text";
    cInput.name = fields[i];
    cInput.id = fields[i];
    cInput.placeholder =
      fields[i] === "First Name"
        ? "John"
    ...
    ///add classes
    cInputDiv.classList.add("input-div");
    cInputDiv.id = fields[i];
    //append

    cInputDiv.append(cLabel, cInput);

    if (fields[i] === "Email") {
      let cEmailDiv = document.createElement("div");
      cEmailDiv.classList.add("email-div");
      let cInfoP = document.createElement("p");
      cInfoP.style.fontSize = "14px";
      cInfoP.style.textAlign = "start";
      cInfoP.innerHTML = "We never sell or give your information away";
      cEmailDiv.append(cInputDiv, cInfoP);
      cForm.append(cEmailDiv);
    } else cForm.append(cInputDiv);

    // cErrMsgDiv.style.display = error === fields[i] ? "flex" : "none";
  }
  ///^^^ ---------- First Three Fields --------------^^^ ///
...
let submitForm = (e) => {
  ...
  if (info.firstName === "") {
    cErrP.innerHTML = `Please enter First Name`;
    cErrMsgDiv.append(cErrP, cSquare);
    let inputDivEl = document.getElementById("First Name");
    inputDivEl.append(cErrMsgDiv);
    error = "First Name";
  } else if (info.lastName === "") {
    cErrP.innerHTML = `Please enter Last Name`;
    cErrMsgDiv.append(cErrP, cSquare);
    let inputDivEl = document.getElementById("Last Name");
    inputDivEl.append(cErrMsgDiv);
    error = "last Name";
  } else if (info.email === "") {
    cErrP.innerHTML = `Please enter Email`;
    cErrMsgDiv.append(cErrP, cSquare);
    let inputDivEl = document.getElementById("Email");
    inputDivEl.append(cErrMsgDiv);
    error = "Email";
  } else if (info.speciality === "Please select" || info.speciality === "") {
    ...
    formData.append("organization", info.speciality);
    formData.append("city", info.nearestAirport.name);
    formData.append("country", country);
    formData.append("formId", "uQsYDbVvkVeUHaqX5Mjz");
    formData.append("location_id", "zatHocFapiIe8bvxutEO");

    fetch("https://services.msgsndr.com/forms/submit", {
      headers: {
        accept: "*/*",
